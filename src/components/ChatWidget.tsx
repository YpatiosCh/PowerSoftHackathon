...
function findMenuItem(itemId: string): MenuItemBase | null {
...
function ChatWindow({ onClose, messages, onSendMessage, isLoading }: ChatWindowProps) {
  ...
  return (
    <motion.div
      ...
      className="absolute bottom-16 left-0 w-80 sm:w-96 flex flex-col bg-[#111113] border border-white/10 rounded-2xl shadow-2xl shadow-black/40 overflow-hidden"
...
export function ChatWidget() {
      ...
      switch (function_name) {
        case "add_to_cart": {
          ...
          if (menuItem) {
            ...
            for (let i = 0; i < qty; i++) {
              addToCart({ ...menuItem, finalPrice: menuItem.price || 0, currentDiscount: 0, currentBucket: 'high', isFlashActive: false, lastUpdated: new Date(), regularPrice: menuItem.price || 0 } as any);
            }
        ...
        case "get_cart": {
          if (cart.length === 0) {
            ...
            const total = cart.reduce((sum, cartItem) => {
              const price = cartItem.finalPrice || cartItem.price || 0;
  ...
  const handleSendMessage = async (content: string) => {
    ...
    const newHistory: Message[] = [...conversationHistory, { role: "user", content }];
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: "⚠️ Chat is currently unavailable. The OpenAI API key may not be configured. You can still browse the menu and add items to your cart!" },
        ]);
        setIsLoading(false);
        return;
      }
